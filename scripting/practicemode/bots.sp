stock int CreateBot(int client, bool forceCrouch, const char[] providedName = "",
  ...
  if (g_ClientBots[client].Length >= g_MaxPlacedBotsCvar.IntValue) {
    PM_Message(
        client,
        "你已经添加太多的BOT （%d 个）。请使用 .deletebot 或 .bots 菜单移除一些。",
...
public Action Command_Bot(int client, int args) {
  ...
  GiveBotParams(bot);
  PM_Message(client, "BOT已创建，使用 .nobot 以移除。");
...
public Action Command_RemoveBot(int client, int args) {
  ...
  if (IsPMBot(target)) {
    ...
    if (botIndex >= 0) {
      ...
      PM_Message(client, "您只能踢出您创建的BOT。");
    }
  } else {
    PM_Message(client, "没有找到BOT，请将准星指向你想移除的BOT。");
  }
...
public Action Event_BotDamageDealtEvent(Event event, const char[] name, bool dontBroadcast) {
  ...
  if (IsPMBot(victim) && IsPlayer(attacker)) {
    ...
    PM_Message(attacker, "---> 对BOT %N 造成了 %d  点伤害 (剩余 %d HP)", damage, victim, postDamageHealth);
  }
...
public Action Event_PlayerBlind(Event event, const char[] name, bool dontBroadcast) {
  ...
  if (IsPMBot(client)) {
    ...
    if (IsPlayer(owner)) {
      PM_Message(owner, "---> 对BOT %N 致盲了 %.1f 秒", GetFlashDuration(client), client);
    }
    ...
    for (int i = 1; i <= MaxClients; i++) {
      if (owner != i && IsPlayer(i) && FloatAbs(now - g_LastFlashDetonateTime[i]) < 0.001) {
        PM_Message(i, "---> 对BOT %N 致盲了 %.1f 秒", GetFlashDuration(client), client);
      }
    }
  }
...
public Action Command_SaveBots(int client, int args) {
  ...
  if (!hasCurrentBots) {
    ...
    PM_Message(client, "您不能在尚未添加BOT时保存BOT固定配置。");
    return Plugin_Handled;
  }
...
public Action Command_LoadBots(int client, int args) {
  ...
  delete botsKv;
  PM_MessageToAll("已载入当前地图所保存的BOT站位。");
  return Plugin_Handled;
}
